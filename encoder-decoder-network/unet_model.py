...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x + self.residual_connection(residual_connection) # needs to have the same channel size to add 
        return x  
...
class UpsampleBlock(nn.Module):
    def __init__(self, in_channels, out_channels, interpolate=False):
        ...
        if interpolate: # non learnable upsampling unlike TranposeConv2D
            self.upsample = nn.Sequential(
                nn.Upsample(scale_factor=2, mode="bilinear", align_corners=True),
                nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=1, padding='same')
            )

        else: # learanble Upsampling 
...
class UNET(nn.Module):
    def __init__(self, 
        ...
        # reverse the encoder config for the decoder 
